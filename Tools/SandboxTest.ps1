...
`$ProgressPreference = 'SilentlyContinue'
try {
  Add-AppxPackage -Path '$($vcLibsUwp.pathInSandbox)' -ErrorAction Stop -Verbose
  Add-AppxPackage -Path '$($uiLibsUwp_2_7.pathInSandbox)' -ErrorAction Stop -Verbose
  Add-AppxPackage -Path '$($uiLibsUwp_2_8.pathInSandbox)' -ErrorAction Stop -Verbose
  Add-AppxPackage -Path '$($desktopAppInstaller.pathInSandbox)' -ErrorAction Stop -Verbose
} catch {
  ...
  try {
    # https://github.com/Azure/azure-powershell/issues/17210
  ...
  Repair-WinGetPackageManager -Version $versionTag -Verbose
}
