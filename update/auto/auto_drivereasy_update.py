def run(*, is_main=(__name__ == "__main__")):
    ...
    files, urls, packages = check_mismatches("manifests\\e\\Easeware\\DriverEasy")
    for i in range(len(urls) - 1, -1, -1):
        if not urls[i].endswith(".exe"):
            del files[i]
            del urls[i]
            del packages[i]
    log(f"Found {len(packages)} DriverEasy installer hash mismatch(es) to inspect.")
    changes = []
    ...
    log(f"Resolved {len(new_versions)} DriverEasy version(s).")
